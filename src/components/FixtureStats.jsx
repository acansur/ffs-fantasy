...
// Bir maçın /fixtures/players verisindeki TÜM oyuncu istatistik alanlarını
// gösteren yeniden kullanılabilir bileşen. Maç, `findFixture` (async → fixture
// nesnesi) ile bulunur; böylece farklı sayfalar farklı maç seçebilir.
...
export default function FixtureStats({ title, subtitle, findFixture }) {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        // 1) Maçı bul (sayfaya özel)
        const fx = await findFixture()
        if (!fx?.fixture?.id) throw new Error('Maç bulunamadı.')
        ...
        for (const t of teams) {
          ...
          for (const p of t?.players || []) {
            ...
            groups[group].push({ id: p?.player?.id, name: p?.player?.name || '—', team: teamName, stats: st })
          }
        }
  ...
  return (
    <div className="stx">
      <h1 className="stx-title">{title}</h1>
      <p className="stx-sub">{subtitle}</p>
